...
% grid with cost of every state combination
LQR_cost = zeros(N_x1, N_x2);
% LQR control for every state combination
...
for i = 1:N_x1
    for j = 1:N_x2
        ...
        % Cost of the current state x
        LQR_cost(i,j) = x'*Q*x;
        % optimal control for the current state x
        LQR_u(i,j) = -K*x;
    end
end
    ...
    for i1 = 1:N_x1
        for i2 = 1:N_x2
            ...
            for j = 1:N_u
                ...
                % INSERT YOUR CODE HERE (use project.m):
                xEnd = output.value;
                i1_next = project(xEnd(1), x1_values);
                i2_next = project(xEnd(2), x2_values);
                ...
                if i1_next <= 0 || i1_next > N_x1 || i2_next <= 0 || i2_next > N_x2
                    ...
                    % INSERT YOUR CODE HERE:
                    % cost of using this control at the current state
                    cost = x_k'*Q*x_k + J_cost(i1_next, i2_next);
                end
                % If this cost better than what was found before:
                if cost < J_new(i1,i2)
%                   INSERT YOUR CODE HERE:
                    % update best cost found so far
                    J_new(i1,i2) = cost;
                    % update corresponding best control found so far
                    u_map(i1,i2) = u_k;
                end
            end
        end
